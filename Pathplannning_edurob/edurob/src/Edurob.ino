...
// #define read_file_mode  //Use if you want to get the World pose and Speed from File 
...
// Matrix
#ifndef OMNI3 //Kinematic Matrix for Robots with 4 Motors
...
#endif
#ifdef OMNI3 //Kinematic Matrix for Robots with 3 Motors
MatrixXd kinematik(3, 3);     // Kinematics matrix 
MatrixXd kinematikInv(3, 3);  // Inverse Kinematics matrix 
Vector3d wheelSpeedSetpoint;  // Wheel speeds in rad/s
Vector3d robotWheelSpeed;     // Current wheel speeds
#endif
...
double rosQuaternion[4];      // Quaternion for ROS transform message1


#ifdef read_file_mode
// Counter for the CSV
static const fileSampleTime = 50 //files Sample Time in ms

//Read one Row of the CSV file as a String and extract the numbers from it.
void getFloatsFromString(char * input, double * output, char delimiter){ 
  char *token = strtok(input, ",");
  int counter = 0;
  while (token != NULL) {
      output[counter] = atof(token);
      token = strtok(NULL, ",");
      counter++;
  }
...
//Print content of Eigen::VectorxXd
void print_vtxd(const Eigen::VectorXd &X) {
  int i, nrow;
  nrow = X.rows();
...
void speedControllerTask(void *pvParameters) {
  ...
  //----------------Low Pass Filter---------------- 
  float smoothData[NumMotors];
  float LPF_Beta = 0.025; // 0<ß<1
  //----------------------------------------------- 

  #ifdef read_file_mode
  char csvFile[] = "/path_planner.csv";
  Serial.print("Loading config file: ");
  Serial.println(csvFile);

  File robotPathPlan = SPIFFS.open(csvFile, "r");

  if (!robotPathPlan || robotPathPlan.isDirectory()) {
    Serial.println("Failed to open config file");
    return;
  }
  //----initialize the Robot Odometrie with the Position in Worldcoordinates---//
  double output[6];
  char input[150];
  robotPathPlan.readBytesUntil('\n',input, 150);  //Read first line of the CSV file 
  getFloatsFromString(input, output, ',');        //Extract first points from CSV 
  robotOdom << output[0], output[1], output[2];   //Safe first points as initial odometrie
  int slowerFrequenz = 0;
  ...
  while (true) {
    ...
    #ifdef read_file_mode
    if(slowerFrequenz >= fileSampleTime / sampleTime){ //Adapt the SampleTime of the Data stored in the CSV to the SampleTime of the SpeedController
      if(robotPathPlan.available()){
        double output[6];
        char input[150];
        robotPathPlan.readBytesUntil('\n',input, 150);
        getFloatsFromString(input, output, ',');
        Vector3d soll_pos;
        soll_pos << output[0], output[1], output[2]; //get the set position from the file
        Vector3d soll_vel;
        soll_vel << output[3], output[4], output[5]; //get the set velocity from the file


        // Error between set position and real position
        Vector3d soll_ist_pos;
        soll_ist_pos = soll_pos - robotOdom;

        //Intended velocity in world coordinate system with consideration of the error and constant factor how much the error should be taken into account
        Vector3d WKStellV;
        WKStellV << soll_vel[0] + soll_ist_pos[0] * 0.5, 
                    soll_vel[1] + soll_ist_pos[1] * 0.5,
                    soll_vel[2] + soll_ist_pos[2] * 0.5;  

        //Intended velocity in worldcoordinateframe without consideration of the error 
        // WKStellV << soll_vel[0], 
        //             soll_vel[1],
        //             soll_vel[2];  

        //transform intendet velocity from world- to robotcoordinates 
        robotSpeed = world_vel_to_robot_vel(robotOdom[2],WKStellV); //with robot odom
        // robotSpeed = world_vel_to_robot_vel(soll_pos[2],WKStellV); //with value read from file
    ...
    #endif //read_file_mode

    for (int i = 0; i < 3; i++) {

      #ifndef read_file_mode //Ac/deceleration Ramp when not reading from file 
    ...
    for (int i = 0; i < NumMotors; i++) {
      speedControllerParam[i].setpoint = wheelSpeedSetpoint(i, 0); // Set wheelSpeedSetpoint as setpoint of PID-Controller
      int64_t encoderVal = WheelEncoder[i].getCount();
      encoderNew[i] = EncoderDir[i] * encoderVal; // Get last Encodercounter 

      // sliding Window filter
      inputsum[i] -= inputs[i][windowIndex];
      inputs[i][windowIndex] = ((((encoderNew[i] - encoderOld[i]) * incrementsToRad) / sampleTime) * 1000); //Encodervalues to wheelvelocity
      inputsum[i] += inputs[i][windowIndex];

      //simple low pass filter
      // smoothData[i] = smoothData[i] - (LPF_Beta * (smoothData[i] - ((((encoderNew[i] - encoderOld[i]) * incrementsToRad) / sampleTime) * 1000))); //low pass filter without Sliding window filter
      smoothData[i] = smoothData[i] - (LPF_Beta * (smoothData[i] - (inputsum[i] / windowSize))); //Output of the Sliding Window filter as input of low pass filter

      // speedControllerParam[i].input = inputsum[i] / windowSize;  // Sliding window as Input of PID-controller
      speedControllerParam[i].input = smoothData[i];  // Low pass filter as Input of PID-controller
    ...
    for (int i = 0; i < NumMotors; i++) //Wheelvelocity
    {
      robotWheelSpeed[i] = inputs[i][windowIndex];
    }


    robotOdomSpeed = (wheelRadius * kinematikInv * robotWheelSpeed); // Conversion wheelvelocity to velocity in robotcoordinates
    ...
    //Conversion Velocity in worldcoordinates to pose in worldcoordinates 
    ...
    windowIndex = (windowIndex + 1) % windowSize; // Index for sliding window filter
    #ifdef read_file_mode
      slowerFrequenz++; //counter for sampling frequenz to filesampling frequenz 
    #endif
...
// print der kinematic und inversekinematic for the selected modell
  Serial.println("Kinematik: ");
  print_mtxd(kinematik);
  Serial.println("Invkinematik: ");
  print_mtxd(kinematikInv);
